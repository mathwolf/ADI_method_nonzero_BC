...
table_data = zeros(6,6);

% Check two different exponential test functions
for r = 1:3
    if r == 1
        disp('x+2t+3z+4t');
        ...
        for j = 2:N
            for k = 2:N
                ...
                if perturbation == FULL            
                    btf = U_next(1,j,k) - (tau/(2*h^2)) * ...
                        (U_next(1,j,k-1) - 2*U_next(1,j,k) + U_next(1,j,k+1)) ...
                        - (tau/(2*h^2)) * ...
                        (U_next(1,j-1,k) - 2*U_next(1,j,k) + U_next(1,j+1,k)) ...
                        + (tau^2/(4*h^4)) * ...
                        (U_next(1,j-1,k-1) - 2*U_next(1,j,k-1) + U_next(1,j+1,k-1) ...
                        -2*U_next(1,j-1,k) + 4*U_next(1,j,k) - 2*U_next(1,j+1,k) ...
                        + U_next(1,j-1,k+1) - 2*U_next(1,j,k+1) + U_next(1,j+1,k+1)) ...
                        + (tau/(2*h^2)) * ...
                        (U(1,j,k-1) - 2*U(1,j,k) + U(1,j,k+1)) ...
                        + (tau/(2*h^2)) * ...
                        (U(1,j-1,k) - 2*U(1,j,k) + U(1,j+1,k)) ...
                        - (tau^2/(4*h^4)) * ...
                        (U(1,j-1,k-1) - 2*U(1,j,k-1) + U(1,j+1,k-1) ...
                        -2*U(1,j-1,k) + 4*U(1,j,k) - 2*U(1,j+1,k) ...
                        + U(1,j-1,k+1) - 2*U(1,j,k+1) + U(1,j+1,k+1)) ...
                        + U(1,j,k);
                    btl = U_next(N+1,j,k) - (tau/(2*h^2)) * ...
                        (U_next(N+1,j,k-1) - 2*U_next(N+1,j,k) + U_next(N+1,j,k+1)) ...
                        - (tau/(2*h^2)) * ...
                        (U_next(N+1,j-1,k) - 2*U_next(N+1,j,k) + U_next(N+1,j+1,k)) ...
                        + (tau^2/(4*h^4)) * ...
                        (U_next(N+1,j-1,k-1) - 2*U_next(N+1,j,k-1) + U_next(N+1,j+1,k-1) ...
                        -2*U_next(N+1,j-1,k) + 4*U_next(N+1,j,k) - 2*U_next(N+1,j+1,k) ...
                        + U_next(N+1,j-1,k+1) - 2*U_next(N+1,j,k+1) + U_next(N+1,j+1,k+1)) ...                        
                        + (tau/(2*h^2)) * ...
                        (U(N+1,j,k-1) - 2*U(N+1,j,k) + U(N+1,j,k+1)) ...
                        + (tau/(2*h^2)) * ...
                        (U(N+1,j-1,k) - 2*U(N+1,j,k) + U(N+1,j+1,k)) ...
                        - (tau^2/(4*h^4)) * ...
                        (U(N+1,j-1,k-1) - 2*U(N+1,j,k-1) + U(N+1,j+1,k-1) ...
                        -2*U(N+1,j-1,k) + 4*U(N+1,j,k) - 2*U(N+1,j+1,k) ...
                        + U(N+1,j-1,k+1) - 2*U(N+1,j,k+1) + U(N+1,j+1,k+1)) ...
                        + U(N+1,j,k);
    ...
    %     end
        ...
        for i = 2:N
            for k = 2:N
                ...
                % term
                if perturbation == FULL            
                    btf = U_next(i,1,k) - (tau/(2*h^2)) * ...
                        (U_next(i,1,k-1) - 2*U_next(i,1,k) + U_next(i,1,k+1)) ...
                        + (tau/(2*h^2)) * ...
                        (U(i,1,k-1) - 2*U(i,1,k) + U(i,1,k+1)) ...
                        - U(i,1,k);
                    btl = U_next(i,N+1,k) - (tau/(2*h^2)) * ...
                        (U_next(i,N+1,k-1) - 2*U_next(i,N+1,k) + U_next(i,N+1,k+1)) ...
                        + (tau/(2*h^2)) * ...
                        (U(i,N+1,k-1) - 2*U(i,N+1,k) + U(i,N+1,k+1)) ...
                        - U(i,N+1,k);
                elseif perturbation == PARTIAL
                    btf = U_next(i,1,k) - (tau/(2*h^2)) * ...
                        (U_next(i,1,k-1) - 2*U_next(i,1,k) + U_next(i,1,k+1)) ...
